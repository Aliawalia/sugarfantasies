class TapeDeck extends HTMLElement {
    dir = "https://file.garden/Zj1sfPoh-G9Y8BHe/";
    ...
    connectedCallback() {
        ...
        if (this.hasAttribute("shuffle")) {
            let thisi = this.list.length,
                randi;
            while (thisi > 0)
                (randi = Math.floor(Math.random() * thisi--)),
                    ([this.list[thisi], this.list[randi]] = [this.list[randi], this.list[thisi]]);
        }
        // play button
